...
func (r *Repository) ListShipments(ctx context.Context, filter model.ShipmentFilter) ([]model.Shipment, error) {
	...
	if filter.ClientID != "" && filter.ClientPhone != "" {
		args = append(args, filter.ClientID, filter.ClientPhone)
		conditions = append(conditions, fmt.Sprintf("(client_id = $%d OR sender_phone = $%d)", len(args)-1, len(args)))
		...
		conditions = append(conditions, fmt.Sprintf("sender_phone = $%d", len(args)))
	}
